      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        {projects.map((project, index) => (
          <div key={index} className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-24 relative">
            {/* Project Image */}
            <div
              className={`lg:col-span-7 order-2 ${project.position === 'right' ? 'lg:order-1' : 'lg:order-2'}`}
            >
            ...
            <div className={`lg:col-span-5 lg:order-${project.position === 'right' ? 'lg:order-2' : 'lg:order-1'} order-1 flex flex-col justify-center`}>
              ...
              <div className={`bg-[#112240] p-6 rounded-md shadow-xl z-20 mb-6 relative w-full lg:w-[120%] ${project.position === 'right'
                ...
                <p className="text-[#ccd6f6] text-sm leading-relaxed">
                  {project.description.split(' ').map((word, i, arr) => {
                    if (word === 'on' && i < arr.length - 1 && arr[i + 1] === 'Visual') {
                      return (
                        <React.Fragment key={i}>
                          ...
                          , <Link href="#" className="text-teal-400 hover:underline">Package Control</Link>, <Link href="#" className="text-teal-400 hover:underline">Atom Package Manager</Link>, and npm.
                        </React.Fragment>
                      );
                    } else if (i > 0 && arr[i - 1] === 'on') {
                      return null;  // Skip words that are part of the link text
                    } else if (word === 'Package' || word === 'Control' || word === 'Atom' || word === 'Manager' || word === 'and' || word === 'npm.') {
                      return null;  // Skip words that are part of other links
                    } else {
                      return `${word} `;
                    }
                  })}
